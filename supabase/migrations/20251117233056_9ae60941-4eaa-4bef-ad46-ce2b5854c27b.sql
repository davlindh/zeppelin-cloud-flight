-- Extend get_unified_admin_dashboard_stats with funding and events metrics
...
BEGIN
  SELECT jsonb_build_object(
    ...
    'funding', jsonb_build_object(
      'total_raised_all_time', COALESCE((
        SELECT SUM(raised_amount) FROM funding_campaigns
      ), 0),
      'total_raised_30d', COALESCE((
        SELECT SUM(amount) FROM donations 
        WHERE status = 'succeeded' 
        AND created_at >= CURRENT_DATE - 30
      ), 0),
      'active_campaigns', (
        SELECT COUNT(*) FROM funding_campaigns 
        WHERE status = 'active'
      ),
      'campaigns_successful', (
        SELECT COUNT(*) FROM funding_campaigns 
        WHERE status = 'successful'
      ),
      'avg_campaign_success_rate', COALESCE((
        SELECT AVG(
          CASE 
            WHEN target_amount > 0 
            THEN (raised_amount::float / target_amount::float) * 100 
            ELSE 0 
          END
        )::numeric(10,2)
