...
export const PersistentPlayer: React.FC = () => {
  ...
  // Synchronize media elements with context state
  useEffect(() => {
    const currentElement = currentMedia?.type === 'video' ? videoRef.current : audioRef.current;
    
    if (!currentElement || !mediaUrl) return;

    // Set the source
    if (currentElement.src !== mediaUrl) {
    ...
    if (isPlaying && currentElement.paused) {
      currentElement.play().catch((error) => {
        ...
        setError('Could not play media. Please check the file format.');
      });
    } else if (!isPlaying && !currentElement.paused) {
      currentElement.pause();
    }

    // Clear errors when media loads successfully
    if (currentElement.readyState >= 2) {
      setError(null);
    }

    // Sync volume
    currentElement.volume = isMuted ? 0 : volume / 100;
    currentElement.playbackRate = playbackRate;

    // Event listeners for synchronization
    const handleTimeUpdate = () => {
      if (currentElement.duration) {
        const newProgress = (currentElement.currentTime / currentElement.duration) * 100;
        // Avoid dispatching if progress hasn't changed significantly to prevent loops
    ...
    const handleLoadedMetadata = () => {
      ...
      if (currentElement.duration !== duration) {
        setDuration(currentElement.duration);
      }
    };

    const handleEnded = () => {
      // Auto-advance to next media if queue has items
      if (queue.length > 1) {
        nextMedia();
      } else {
        // Stop playback if no more items in queue
        togglePlay();
      }
    };
    ...
    const handleError = (e: Event) => {
      ...
      setError('Media playback failed. Please try again.');
    };

    currentElement.addEventListener('timeupdate', handleTimeUpdate);
    currentElement.addEventListener('loadedmetadata', handleLoadedMetadata);
    currentElement.addEventListener('ended', handleEnded);
    currentElement.addEventListener('error', handleError);

    return () => {
      currentElement.removeEventListener('timeupdate', handleTimeUpdate);
      currentElement.removeEventListener('loadedmetadata', handleLoadedMetadata);
      currentElement.removeEventListener('ended', handleEnded);
      currentElement.removeEventListener('error', handleError);
    };
  }, [currentMedia, mediaUrl, isPlaying, volume, isMuted, playbackRate, progress, duration, queue.length, nextMedia, togglePlay, seekTo, setDuration, setError]);
  ...
  const handleSeek = (value: number) => {
    ...
    if (currentElement && currentElement.duration) {
      const newTime = (value / 100) * currentElement.duration;
      currentElement.currentTime = newTime;
    }
    
    seekTo(value);
  };

  // Auto-cache media when it starts playing
  useEffect(() => {
  ...
  return (
    <>
      {/* Hidden media elements for actual playback */}
      ...
      <div className={cn(
        ...
        <Card className="card-enhanced border-0 shadow-glow bg-background/95 backdrop-blur-md overflow-hidden">
          ...
          {/* Error Display */}
          {error && (
            <div className="px-3 py-2 bg-destructive/10 border-l-2 border-l-destructive text-destructive text-sm">
              {error}
              <button 
