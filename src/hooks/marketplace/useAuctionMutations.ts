...
export const useAuctionMutations = () => {
  ...
  const createAuction = async (auctionData: CreateAuctionData): Promise<Auction | null> => {
    ...
    try {
      ...
      const dbData = {
        ...
        slug: generateSlug(auctionData.title),
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      };

      console.log('🔄 Creating auction with data:', dbData);
      ...
      console.error('❌ Create auction error:', errorMessage);
  ...
  const updateAuction = async (auctionData: UpdateAuctionData): Promise<Auction | null> => {
    ...
    try {
      ...
      if (auctionData.title) {
        updateFields.title = auctionData.title;
        updateFields.slug = generateSlug(auctionData.title);
      }
      if (auctionData.description) updateFields.description = auctionData.description;
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      console.error('❌ Update auction error:', errorMessage);
      setError(errorMessage);
      return null;
    } finally {
      setIsLoading(false);
  ...
  const endAuction = async (auctionId: string, force: boolean = false): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      console.error('❌ End auction error:', errorMessage);
      setError(errorMessage);
      return false;
    } finally {
      setIsLoading(false);
  ...
  const extendAuction = async (auctionId: string, additionalMinutes: number): Promise<boolean> => {
    ...
    try {
      console.log('🔄 Extending auction:', auctionId, 'by', additionalMinutes, 'minutes');
      ...
      console.log('✅ Auction extended successfully:', auctionId, 'new end time:', newEndTime);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      console.error('❌ Extend auction error:', errorMessage);
      setError(errorMessage);
      return false;
    } finally {
      setIsLoading(false);
  ...
  const deleteAuction = async (auctionId: string): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Unknown error occurred';
      console.error('❌ Delete auction error:', errorMessage);
      setError(errorMessage);
      return false;
    } finally {
      setIsLoading(false);
