...
export const OptimizedUnifiedSearchBar: React.FC<OptimizedUnifiedSearchBarProps> = ({
  ...
  // Debounce the search term to prevent excessive re-renders
  ...
  const suggestions = useMemo(() => {
    ...
    // Add matching product titles (limit to prevent performance issues)
  ...
  }, [debouncedSearchTerm, state.searchTerm, dispatch]);
    ...
    <div ref={searchRef} className={`relative ${className}`}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-background border border-border rounded-xl shadow-xl max-h-[80vh] sm:max-h-96 overflow-y-auto">
          ...
          {/* Recent searches */}
          {!localSearchTerm && recentSearches.length > 0 && (
