...
import { withTimeout, logError } from '@/utils/adminApi';
...
export const AdminSponsorForm = ({ onClose, sponsorId }: SponsorFormProps) => {
  ...
  const onSubmit = async (data: SponsorFormData) => {
    ...
    try {
      console.log('Starting sponsor creation...', data);

      let logoUrl = null;
      if (logoFile) {
        console.log('Uploading logo...');
        logoUrl = await withTimeout(uploadLogo(logoFile), 15000);
      ...
      const errorMessage =
        err instanceof Error ? err.message : (typeof err === 'string' ? err : 'Failed to create sponsor');
