...
// Utility function to extract storage path from full Supabase URL
const extractStoragePath = (fullUrl: string): string => {
  ...
  const bucketIndex = pathParts.indexOf('object') + 2; // object/public/BUCKET
  if (bucketIndex >= 0 && bucketIndex < pathParts.length) {
    // Return everything after bucket name (the relative path)
  ...
  project: {
    ...
    fromForm: (data: Record<string, unknown>) => {
      ...
      return {
        ...
        image_path: extractStoragePath(imagePath)
      };
    }
  },
...
export const AdminFormFactory: React.FC<AdminFormFactoryProps> = ({
  ...
  const { ensureAdminAuth, isAdmin } = useAdminAuth();
  ...
  const handleSubmit = async (data: Record<string, unknown>) => {
    // Ensure admin auth session is active before database operations
    if (isAdmin) {
      const authEnsured = await ensureAdminAuth();
      if (!authEnsured) {
        throw new Error('Failed to ensure admin authentication. Please try again.');
      }
    }
